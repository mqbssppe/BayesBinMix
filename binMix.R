...
allocationSamplerBinMix <- function(Kmax, alpha,beta,gamma,m,burn,data,thinning,z.true,ClusterPrior,ejectionAlpha,Kstart,kmeansInit,outputDir,metropolisMoves,reorderModels){
...
#	end of reallocation proposal from conditional probs
		...
		if(iter %% (m/100) == 0){
			if(length(kValues) > 0){
				plot(kValues)
				#points(rep(iter,K),p,col = 1:K,pch = K)
			}
